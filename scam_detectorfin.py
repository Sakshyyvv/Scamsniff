...
if not os.path.exists(report_file):
    with open(report_file, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["handle","user_id", "user_experience", "proof_filename"])
...
if os.path.exists(report_file):
    reports_df = pd.read_csv(report_file)
    latest_reports = reports_df.tail(3)
    st.markdown("#### 📰 Latest Scam Activity")
    for idx, row in latest_reports.iterrows():
        st.markdown(
            f"<div style='background:#003366;border-left:4px solid #000000;padding:8px;margin-bottom:6px;'>"
            f"<b>{row['handle']}</b>: {row['user_experience'][:]}"
            ...
            if scam_flag:
                ...
                if not reported.empty:
                    ...
                    for idx, rpt in reported.iterrows():
                        st.markdown(
                            f"<div style='background-color:#A37676; padding:10px; border-radius:10px; margin:5px 0;'>"
                ...
                st.markdown("### ✅ Why This Page is Genuine:")
                reasons = []
                if contact == "YES":
                    reasons.append("✅ Contact information is provided.")
                if website == "YES":
                    reasons.append("✅ Official website link is available.")
                if comments_status == "ON":
                    reasons.append("✅ Comments are enabled (users can interact).")
                if payment == "COD":
                    reasons.append("✅ Payment method is Cash on Delivery (low risk).")
                if tags == "YES":
                    reasons.append("✅ Tagged by customers (community trust).")
...
# --------------------------- SITUATION 2: Handle Not Found ----------------------------
        ...
        contact = st.selectbox("Legit Contact Info Available?", ["YES", "NO"])
        website = st.selectbox("Legit Website Link Available?", ["YES", "NO"])
        comments = st.selectbox("Comments Status", ["ON", "OFF"])
        tags = st.selectbox("Tagged by Any Customers?", ["YES", "NO"])
